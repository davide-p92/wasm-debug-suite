...
pub struct SymbolInfo {
    pub name: String,
    pub category: String,   // e.g. "function", "global", "dwarf_var"
...
*/
        ...
        // Symbole nach der Initialisierung einlesen
        runtime.populate_symbols();
...
/*    pub fn get_global_value(&self, name: &str) -> Option<i32> {
    ...
    pub fn populate_symbols(&self) {
        let mut table = self.symbol_table.borrow_mut();
        table.clear();

        // 1.Exportierte Funktionen
        for (name, func) in self.exported_functions.iter() {
            table.insert(
                name.clone(),
                SymbolInfo {
                    name: name.clone(),
                    category: "export function".into(),
                    type_name: Some(format!("{:?}", func.ty(&self.store.borrow()))),
                    address: None,
                },
            );
        }

        // 2.Exportierte Globals
        for (name, global) in self.exported_globals.iter() {
            let ty = format!("{:?}", global.ty(&self.store.borrow()).ty);
            table.insert(
                name.clone(), 
                SymbolInfo {
                    name: name.clone(),
                    category: "global variable".into(),
                    type_name: Some(ty),
                    address: None,
                },
            );
        ...
        // 3.DWARF-Variablen (falls vorhanden)
    ...
    pub fn dump_symbols(&self) {
        ...
        println!("📜 Symbol Table ({} entries):", table.len());
        /*for (name, desc) in table.iter() {
            println!("   {:<20} → {}", name, desc);
        }*/
        println!("{:<20} {:<18} {:<15} {:<10}", "Name", "Category", "Type", "Address");
        println!("{}", "-".repeat(70));

        for sym in table.values() {
            let type_str = sym.type_name.clone().unwrap_or_else(|| "-".into());
            let addr_str = match sym.address {
                Some(a) => format!("0x{:X}", a),
                None => "-".into(),
            };
            println!(
                "{:<20} {:<18} {:<15} {:<10}",
                sym.name, sym.category, type_str, addr_str
            );
        }
    }
